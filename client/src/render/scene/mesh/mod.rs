...
macro_rules! impl_mesh {
    ($ty:ty, |$mesh:pat, $gl:pat| $draw:tt) => {
        impl AbstractPreparedMesh for $ty {
            fn positions(&self) -> &util::FloatBuffer { &self.positions }
            fn normals(&self) -> &util::FloatBuffer { &self.normals }
            fn tex_pos(&self) -> &util::FloatBuffer { &self.tex_pos }
            fn tex_sprite_number(&self) -> &[usize] { &self.tex_sprite_number }
            fn tex_offset(&self) -> &util::FloatBuffer { &self.tex_offset }
...
impl IndexedMesh {
    /// Triplets of floats indicating the unit model position.
    pub fn positions(&self) -> &[f32] { self.mesh.positions() }

    /// Triplets of floats indicating the unit model position.
    pub fn positions_mut(&mut self) -> &mut Vec<f32> { self.mesh.positions_mut() }

    /// Triples of floats indicating the unit normal of faces.
    pub fn normals(&self) -> &[f32] { self.mesh.normals() }

    /// Triples of floats indicating the unit normal of faces.
    pub fn normals_mut(&mut self) -> &mut Vec<f32> { self.mesh.normals_mut() }

    /// Tuples indicating the unit sprite position of the vertice and the internal sprite number.
    ///
    /// See [`Mesh::tex_pos`] for details.
    pub fn tex_pos(&self) -> &[(usize, f32, f32)] { self.mesh.tex_pos() }

    /// Tuples indicating the unit sprite position of the vertice and the internal sprite number.
    ///
    /// See [`Mesh::tex_pos_mut`] for details.
    pub fn tex_pos_mut(&mut self) -> &mut Vec<(usize, f32, f32)> { self.mesh.tex_pos_mut() }
