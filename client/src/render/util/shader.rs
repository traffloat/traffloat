...
pub fn create_shader(
    ...
    let shader = gl.create_shader(shader_type).expect("Failed to initialize WebGL shader");
...
pub fn create_program(
    ...
    let prog = gl.create_program().expect("Failed to initialize WebGL program");
    create_shader(gl, &prog, vert_file, vert_code, WebGlRenderingContext::VERTEX_SHADER);
    create_shader(gl, &prog, frag_file, frag_code, WebGlRenderingContext::FRAGMENT_SHADER);
    ...
    if !value.is_truthy() {
        ...
        panic!("Error linking {}/{}: {}", vert_file, frag_file, log.unwrap_or_default());
    }
