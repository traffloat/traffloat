...
- Health represents the general physical well-being of a resident.
  ...
  It also has impacts on skill attributs defined as health-dependent.
...
Resident behavior is controlled by different AIs.
Some AIs are selectable by the player,
while others automatically override player control under specific circumstances.

### Compliance

"Compliance" is a core attribute representing the willingness of a resident to comply with player commands.
Extreme psychology damages compliance, which is slowly restored over time.

When compliance is low, the resident would randomly stop accepting player instructions.
The non-compliance AI would try to maximize resident happiness
by accessing facilities and fluids that increase happiness.
...
When morality is low, the resident would be controlled by a crime AI that commits random crimes.
...
Mods can add hooks to increase resident attributes such as infamy when crime are committed.

### Security

Residents with "restraint" cargo in their inventory may restrain a resident
if certain attribute conditions are satisfied (i.e. one can physically overpower the other).
Restrained residents are controlled by a "restrained" AI that would follow a restrainer to board or alight vehicles.
...
High morality residents may be randomly OR'ed with a new bit, increasing their intimacy compatibility.
...
An enbryo storage facility cannot be constructed by the player.
...
Each resident also has a "genetic code" represented as a random 128-bit mask.
The genetic code of naturally born residents inherit random 64 bits of genetic code from each parent.
