...
- scene: Client-side code for receiving simulation updates and rendering
- client: Client user interaction management
...
which broadcasts updates to `scene` through the [protocol](./protocol/)
in the form of bevy message channels, potentially synced through the network.
`scene` builds a scene world that mirrors the physics world,
